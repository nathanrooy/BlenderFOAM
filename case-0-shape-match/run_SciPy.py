...
def CostFunction(x0):
    ...
    #---Add Log Entry
    opt_log = file("NLopt_POWELL.txt", 'a')
...
#xopt = scipy.optimize.fmin(CostFunction, x0, xtol=1e-4, maxiter=10000, maxfun=10000,disp=1,full_output=1)   # downhill simplex algorithm
xopt = scipy.optimize.fmin_powell(CostFunction, x0, xtol=1e-4, ftol=1e-4, maxiter=10000, maxfun=10000, full_output=1)
#xopt = scipy.optimize.brute(CostFunction, apheRange, Ns=20, full_output=0, finish=None)                  # brute force, Ns=number of samples
#xopt = scipy.optimize.fmin_bfgs(CostFunction,x0,full_output=1,maxiter=10000,disp=1)
#xopt = scipy.optimize.anneal(CostFunction,x0,schedule='fast',full_output=1,lower=-2.048,upper=2.048,Tf=0,maxiter=10000,dwell=100)   #schedule='fast','cauchy','boltzmann'
#xopt = scipy.optimize.fmin_slsqp(CostFunction,x0,f_eqcons=None,f_ieqcons=None,bounds=[(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),(-0.5,1.0),],fprime=None,fprime_eqcons=None,fprime_ieqcons=None,full_output=1)
#xopt = scipy.optimize.fmin_l_bfgs_b(CostFunction,x0,approx_grad=1,m=10,maxfun=10000,disp=1)
